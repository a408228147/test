...
　　　　　？：0或一个
 ...
     vector是线程安全的ArrayList，在内存中占用连续的空间。有一个初始大小，当数据条大于这个初始大小后会重写分配一个更大的连续空间。如果vector定义为object则可以存放任意类型
     
     try{}catch{}会增加额外的开销
     
     接口中什么的‘变量’必须为public final static，所以为常量
     
     子类可以访问父亲受保护的成员
     
     抽象类不能被final修饰，因为被final修饰的不能被继承，而抽象类必须被继承才有意义。
     抽象方法不能被final修饰，因为被final修饰的方法不能被重写，也不能有具体的实现方法，但是在抽象类中的非抽象方法可以有具体的方法。
